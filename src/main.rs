...
const EFI_GRAPHICS_OUTPUT_PROTOCOLGUID: EfiGuid = EfiGuid {
    data0: 0x9042a9de,
    data1: 0x23dc,
    data2: 0x4a38,
...
fn locate_graphic_protocol<'a>(
    ...
    let status = (efi_system_table.boot_services.locate_protocol)(
        &EFI_GRAPHICS_OUTPUT_PROTOCOLGUID,
        null_mut::<EfiVoid>(),
...
trait Bitmap {
    ...
    /// # Safety
    /// Returend pointer is valid as long as the given coordinates are valid.
...
unsafe fn unchecked_draw_pint<T: Bitmap>(buf: &mut T, color: u32, x: i64, y: i64) {
...
fn fill_rect<T: Bitmap>(buf: &mut T, color: u32, px: i64, py: i64, w: i64, h: i64) -> Result<()> {
    ...
    for y in py..py + h {
        for x in px..px + w {
            // SAFETY: checked above
            unsafe { unchecked_draw_pint(buf, color, x, y) };
        }
    }
    Ok(())
}
